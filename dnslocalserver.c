...
int main(int argc, char *argv[]){
    ...
    for (;;){ /* Run forever */
        ...
        int client_fd = accept(sock,(struct sockaddr *)&clientaddr,&cliAddrLen);
        ...
        received_length = recv(client_fd,recvBuff,sizeof(recvBuff),0);
        ...
        send(client_fd, sendBuff, send_length, 0); 
...
void processReceivedData(){
    ...
    received_dup_length = received_length;
    ...
    for(i = 0; i < QCount; i++)
    {
        ...
        if(strlen(answer[i]) == 0)
        {
            ...
            {  
                ...
                printf("TLD:%s\n", TLD_domain[level]);
                printf("%d\n", strlen(TLD_domain[level]));
                if(strcmp(TLD_domain[level], "arpa") == 0)
                {
                    int Rsockfd;
                    ...
                    inet_pton(AF_INET, "127.0.0.1", &Rservaddr.sin_addr);
                    Rservaddr.sin_port = htons(53);
                    
                    //connect(sockfd, (struct sockaddr *)&servaddr, sizeof(servaddr));
                    sendto(Rsockfd, recvBuff_dup, received_dup_length, 0, (struct sockaddr *)&Rservaddr, sizeof(Rservaddr));  
                ...
                else
                {
                    ...
                    char tmp[MAX_SIZE];
                    memcpy(&tmp, &recvBuff_dup, 12);
                    char answeri[MAX_SIZE];
                    memcpy(&answeri, &recvBuff_dup[start[i]], stop[i] -start[i]);
                    unsigned short Q = htons(0x0001);
                    memcpy(&tmp[4], &Q,sizeof(unsigned short));
                    memcpy(&tmp[12], &answeri, stop[i] - start[i]);
                    sendto(Rsockfd, tmp, 12 + stop[i] - start[i], 0, (struct sockaddr *)&Rservaddr, sizeof(Rservaddr));  
            ...
            else // Iterative 
            {
                printf("Iterative\n");
                int Rsockfd;
                char tmpBuff[MAX_SIZE];
                int tmp_length = totalLength;
                struct sockaddr_in Rservaddr;
                Rsockfd = socket(AF_INET, SOCK_DGRAM, 0);
                bzero(&Rservaddr, sizeof(Rservaddr));
                Rservaddr.sin_family = AF_INET;
                ...
                int tmp_tmp = 0;
                tmp_length = recvfrom(Rsockfd, tmpBuff, sizeof(tmpBuff), 0, NULL, NULL);
                tmp_tmp = tmp_length;
                printf("tmp_length: %d\n", tmp_length);
                //cache RR
                close(Rsockfd);
                ...
                unsigned short flag = (0x0000);
                memcpy(&flag, &tmpBuff[2],sizeof(unsigned short));
                unsigned short RCode = htons(flag) & (0x0003);
                printf("RCode: %d\n", RCode);

                unsigned short ACount = (0x0000);
                memcpy(&ACount, &tmpBuff[6],sizeof(unsigned short));
                ACount = htons(ACount);

                unsigned short AuthCount = (0x0000);
                memcpy(&AuthCount, &tmpBuff[8],sizeof(unsigned short));
                AuthCount = htons(AuthCount);
                ...
                while(1)
                {
                    printf("%d\n", (RCode == (0x0003)) && (ACount == 0));
                    if(((RCode == (0x0003)) && (ACount == 0)) || ((RCode == (0x0000)) && (ACount != 0)))
                    {
                        int y = 0;
                        for(y = 0; y < tmp_tmp; y++)
                        {
                            sendBuff[y] = tmpBuff[y];
                        }
                        printf("%d\n", tmp_tmp);
                        sendBuff[tmp_tmp] = '\0';
                        send_length = tmp_tmp;
                        //strcpy(sendBuff, tmpBuff);
                        break;
                    }
                    // Skip answer
                    unsigned char domainLength = (0x00);
                    unsigned char location = (0x0000);
                    memcpy(&domainLength, &tmpBuff[tmp_length], sizeof(unsigned char));
                    tmp_length++;
                    printf("%d\n", tmp_length);
                    printf("    Name: ");
                    while(1){   
                        ...
                        unsigned char data = 0x00;
                        for(j = 0; j < domainLength; j++){
                            memcpy(&data, &tmpBuff[tmp_length], sizeof(unsigned char));
                            tmp_length++;
                            printf("%c", data);
                        }
                    ...
                    unsigned short data_length = (0x0000);
                    memcpy(&data_length, &tmpBuff[tmp_length], sizeof(unsigned short));
                    data_length = htons(data_length);
                    //datalength
                    tmp_length += 2;
                    // data
                    ...
                    char IP[MAX_SIZE];
                    sprintf(IP, "%d.%d.%d.%d", tmpBuff[tmp_length], tmpBuff[tmp_length + 1], tmpBuff[tmp_length + 2], tmpBuff[tmp_length + 3]);
                    ...
                    // dns port: root is 10000, TLD is 10001 and so on
                    sendto(Rsockfd, recvBuff_dup, received_length, 0, (struct sockaddr *)&Rservaddr, sizeof(Rservaddr));  
                    
                    send_length = recvfrom(Rsockfd, tmpBuff, sizeof(tmpBuff), 0, NULL, NULL);
                    printf("while send_length:%d\n", send_length);
                    printf("while send_length:%d\n", strlen(tmpBuff));
                    //cache RR
                    close(Rsockfd);
                ...
                // printf("while out send_length:%d\n", send_length);
                // strncpy(sendBuff, tmpBuff, send_length);
                // printf("%d %d\n", strlen(sendBuff), strlen(tmpBuff));
                close(Rsockfd);
                return;
            }
        }
