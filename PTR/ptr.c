...
void processReceivedData(){
    ...
    for(i = 0; i < QCount; i++)
    {
        ...
        if(strlen(answer[i]) == 0)
        {
            ...
            char query[MAX_SIZE];
            sprintf(query, "%s.%s.%s.%s", TLD_domain[level - 2],TLD_domain[level - 3],TLD_domain[level - 4],TLD_domain[level - 5]);

            FILE* upop;
            int name_length = strlen(query);
            ...
            else
            {
                ...
                printf("TLD: %s\n", query_domain_name_dup);
                domain = strtok(query_domain_name_dup, ".");
                while(domain != NULL)
                {
                    int length = strlen(domain);
                    unsigned char domainLength = length;
                ...
                unsigned short type = htons(0x0c);
